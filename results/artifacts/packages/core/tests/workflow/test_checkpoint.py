...
async def test_memory_checkpoint_storage_list_checkpoints():
    ...
    # workflow-1のlist_checkpoint_idsをテストします。
    ...
    # workflow-1のlist_checkpointsをテストします（オブジェクトを返します）。
    ...
    # workflow-2のlist_checkpoint_idsをテストします。
    ...
    # workflow-2のlist_checkpointsをテストします（オブジェクトを返します）。
    ...
    # ワークフローフィルターなしのlist_checkpointsをテストします（すべてのチェックポイント）。
...
async def test_file_checkpoint_storage_corrupted_file():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # list_checkpointsは破損ファイルを適切に処理するはずです。
...
async def test_file_checkpoint_storage_json_serialization():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # JSONファイルが正しくフォーマットされていることを検証します。
        file_path = Path(temp_dir) / f"{checkpoint.checkpoint_id}.json"
...
def test_checkpoint_storage_protocol_compliance():
    # このテストは両方の実装が必要なメソッドをすべて持っていることを保証します。
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for storage in [memory_storage, file_storage]:
            # すべてのプロトコルメソッドが存在し呼び出し可能であることをテストします。
