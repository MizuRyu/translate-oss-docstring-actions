...
class TestSerializationWorkflowClasses:
    ...
    def test_edge_serialization(self) -> None:
        """Edge がシリアライズ可能で、正しいフィールドを持つことをテスト。"""
        # 条件なしの edge をテスト。
    ...
    def test_edge_serialization_with_named_condition(self) -> None:
        """名前付き関数条件を持つ Edge が condition_name を正しくシリアライズすることをテスト。"""
    ...
    def test_edge_serialization_with_lambda_condition(self) -> None:
        """ラムダ条件を持つ Edge が condition_name を '<lambda>' としてシリアライズすることをテスト。"""
    ...
    def test_single_edge_group_serialization(self) -> None:
        """SingleEdgeGroup がシリアライズ可能で、edges と type を含む正しいフィールドを持つことをテスト。"""
        ...
        # edgesフィールドが存在し、edgeを含んでいることを検証する
    ...
    def test_fan_out_edge_group_serialization(self) -> None:
        """FanOutEdgeGroupがシリアライズ可能で、edgesやtypeを含む正しいフィールドを持つことをテストする。"""
        ...
        # selection_func_nameフィールドのテスト（選択関数が提供されていない場合はNoneであるべき）
        ...
        # edgesフィールドが存在し、正しいedgesを含んでいることを検証する
        ...
        # JSON内のselection_func_nameフィールドのテスト
    ...
    def test_fan_out_edge_group_serialization_with_selection_func(self) -> None:
        """名前付き選択関数を持つFanOutEdgeGroupがselection_func_nameを正しくシリアライズすることをテストする。"""

        def custom_selector(data: Any, targets: list[str]) -> list[str]:
            """テスト用のカスタム選択関数。"""
            return targets[:1]  # 最初のターゲットのみを選択する
    ...
    def test_fan_out_edge_group_serialization_with_lambda_selection_func(self) -> None:
        """ラムダ選択関数を持つFanOutEdgeGroupがselection_func_nameを'<lambda>'としてシリアライズすることをテストする。"""
        edge_group = FanOutEdgeGroup(
    ...
    def test_fan_in_edge_group_serialization(self) -> None:
        """FanInEdgeGroupがシリアライズ可能で、edgesやtypeを含む正しいフィールドを持つことをテストする。"""
    ...
    def test_switch_case_edge_group_serialization(self) -> None:
        """SwitchCaseEdgeGroupがシリアライズ可能で、edgesやtypeを含む正しいフィールドを持つことをテストする。"""
        cases = [
        ...
        # 最初のケース（SwitchCaseEdgeGroupCase）をチェックする
        ...
        # デフォルトケース（SwitchCaseEdgeGroupDefault）をチェックする
        ...
        # edges内のcondition_nameフィールドをチェックする - SwitchCaseEdgeGroupのedgesは条件を持たない
        # なぜなら条件ロジックはグループレベルのselection_funcで実装されているため
        ...
        # JSONのedges内のcondition_nameフィールドをチェックする - SwitchCaseEdgeGroupではNoneであるべき
    ...
    def test_nested_workflow_executor_serialization(self) -> None:
        """深くネストされたWorkflowExecutors（サブワークフロー内のサブワークフロー）の完全なシリアライズをテストする。

        このテストは、ネストされたWorkflowExecutorオブジェクトが
        完全なワークフロー構造を含めて完全にシリアライズされることを検証する。
        深くネストされたワークフローとすべてのExecutorを含む。
        ...
        # WorkflowExecutorを持つ中間ワークフローを作成する
        ...
        # ネストされたWorkflowExecutorを持つ外側のワークフローを作成する
        ...
        # 中間のWorkflowExecutorが存在し、完全なネストされたワークフローのシリアライズを持つことを検証する
        ...
        # ネストされたワークフローが完全にシリアライズされていることを検証する
        ...
        # JSONシリアライズが完全なネスト構造を保持することをテストする
        json_str = outer_workflow.to_json()
        parsed = json.loads(json_str)

        # JSON内で完全な構造が保持されていることを検証する
        ...
        # JSON内にネストされたワークフローが存在することを検証する
        ...
        # WorkflowExecutorが直接アクセスされた場合にも正しくシリアライズされることをテストする
    ...
    def test_switch_case_edge_group_serialization_with_named_condition(self) -> None:
        """名前付き条件関数を持つSwitchCaseEdgeGroupがcondition_nameを正しくシリアライズすることをテストする。"""
    ...
    def test_workflow_serialization(self) -> None:
        """Workflowがシリアライズ可能で、edgesを含む正しいフィールドを持つことをテストする。"""
        ...
        # JSONシリアライズでedgesが保持されていることを検証する
    ...
    def test_workflow_serialization_excludes_non_serializable_fields(self) -> None:
        """シリアライズ不可能なフィールドがシリアライズから除外されることをテストする。"""
        executor1 = SampleExecutor(id="executor1")
        executor2 = SampleExecutor(id="executor2")

        workflow = WorkflowBuilder().add_edge(executor1, executor2).set_start_executor(executor1).build()

        # model_dumpのテスト - プライベートなランタイムオブジェクトを含まないこと
        data = workflow.to_dict()

        # これらのプライベートなランタイムフィールドはシリアライズデータに含まれてはいけない
    ...
    def test_workflow_name_description_serialization(self) -> None:
        """workflowのnameとdescriptionが正しくシリアライズされることをテストする。"""
        # テスト1: nameとdescriptionあり
        workflow1 = (
        ...
        assert "name" not in data2  # Noneの値は含まれてはいけない
        assert "description" not in data2

        # テスト3: nameのみ（descriptionなし）
    ...
    def test_executor_field_validation(self) -> None:
        """Executorフィールドのバリデーションが正しく機能することをテストする。"""
        # 有効なexecutor
    ...
    def test_edge_field_validation(self) -> None:
        """Edgeフィールドのバリデーションが正しく機能することをテストする。"""
        # 有効なedge
        edge = Edge(source_id="source", target_id="target")
        assert edge.source_id == "source"
        assert edge.target_id == "target"

        # 空のsource_idに対するバリデーション失敗をテストする
        with pytest.raises(ValueError):
            Edge(source_id="", target_id="target")

        # 空のtarget_idに対するバリデーション失敗をテストする
        with pytest.raises(ValueError):
            Edge(source_id="source", target_id="")


def test_comprehensive_edge_groups_workflow_serialization() -> None:
    """SwitchCase、FanOut、FanInのすべてのedge groupタイプを使用するworkflowのシリアライズをテストする。"""
    # 包括的なworkflowのためのexecutorsを作成する
    ...
    # 3つのedge groupタイプすべてを含むworkflowを構築する
    workflow = (
        WorkflowBuilder()
        .set_start_executor(router)
        # 1. SwitchCaseEdgeGroup: 条件付きルーティング
    ...
    # エッジグループが3種類すべてを含むことを検証する
    edge_groups = data["edge_groups"]
    edge_group_types = [eg.get("id", "").split("/")[0] for eg in edge_groups]

    # 含まれるべきもの: SwitchCaseEdgeGroup、SingleEdgeGroup（2つ）、FanOutEdgeGroup、FanInEdgeGroup
    ...
    # シリアライズ不可能なフィールドが除外されていることを検証する
    assert "_runner_context" not in data
    assert "_shared_state" not in data
    assert "_runner" not in data

    # 各edge groupタイプをその構造を調べて識別できることをテストする
    ...
    # 重要な検証はすべてのedge groupタイプが存在しシリアライズ可能であること 個々のedge groupフィールドは実装によって異なる可能性があるが、
    # それぞれ少なくともタイプを識別する'id'フィールドと'edges'フィールドを持つべきである
