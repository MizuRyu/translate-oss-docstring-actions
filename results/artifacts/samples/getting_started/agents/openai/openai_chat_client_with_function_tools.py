...
async def tools_on_agent_level() -> None:
    ...
    # Agent作成時にツールが提供される Agentはその生涯中に任意のクエリでこれらのツールを使用できる
    agent = ChatAgent(
    ...
    # 最初のクエリ - Agentはweatherツールを使用可能
    query1 = "What's the weather like in New York?"
    print(f"User: {query1}")
    result1 = await agent.run(query1)
    print(f"Agent: {result1}\n")

    # 2番目のクエリ - Agentはtimeツールを使用可能
    query2 = "What's the current UTC time?"
    print(f"User: {query2}")
    result2 = await agent.run(query2)
    print(f"Agent: {result2}\n")

    # 3番目のクエリ - 必要に応じて両方のツールを使用可能
...
async def tools_on_run_level() -> None:
    ...
    query2 = "What's the current UTC time?"
    print(f"User: {query2}")
    result2 = await agent.run(query2, tools=[get_time])  # このクエリ用の別のツール
    print(f"Agent: {result2}\n")
...
async def mixed_tools_example() -> None:
    """Agentレベルのツールとrunメソッドのツールの両方を示す例。"""
    print("=== Mixed Tools Example (Agent + Run Method) ===")

    # いくつかの基本ツールで作成されたAgent
    agent = ChatAgent(
        chat_client=OpenAIChatClient(),
    ...
    # Agentは作成時のget_weatherとrunメソッドからの追加ツールにアクセス可能
