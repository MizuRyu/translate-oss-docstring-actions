...
async def main(scenario: Literal["chat_client", "chat_client_stream", "ai_function", "all"] = "all"):
    ...
    # トレーシングを有効にし、指定されたパラメータに基づいて必要なトレーシング、ロギング、メトリクスプロバイダーを作成します。
    setup_observability(
        enable_sensitive_data=True,
        # `OTLP_ENDPOINT` 環境変数が設定されており、下記のものと異なる場合、両方のエンドポイントがOTLPエクスポーターの作成に使用されます。
        # Application Insightsの接続文字列についても同様です。
