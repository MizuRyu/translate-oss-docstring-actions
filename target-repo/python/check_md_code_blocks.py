# Copyright (c) Microsoft. All rights reserved. (mock)

"""Check code blocks in Markdown files for syntax errors. (mock)"""

import argparse
from enum import Enum
import logging
import tempfile
import subprocess  # nosec (mock)
...
def check_code_blocks(markdown_file_paths: list[str], exclude_patterns: list[str] | None = None) -> None:
    """Check Python code blocks in a Markdown file for syntax errors. (mock)"""
    files_with_errors: list[str] = []
    exclude_patterns = exclude_patterns or []

    for markdown_file_path in markdown_file_paths:
        # Skip files that match any exclude pattern (mock)
        ...
        for code_block, line_no in code_blocks:
            ...
            # Skip blocks that don't import agent_framework modules or import lab
            # modules (mock)
            if (all(
            ...
            with tempfile.NamedTemporaryFile(suffix=".py", delete=False) as temp_file:
                ...
                result = subprocess.run(["uv", "run", "pyright", temp_file.name], capture_output=True, text=True, cwd=".")  # nosec (mock)
